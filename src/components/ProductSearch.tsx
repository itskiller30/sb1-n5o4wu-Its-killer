...
import { Search, Filter, SortAsc, MapPin, Star, TrendingUp, Loader2 } from 'lucide-react';
...
const ProductSearch: React.FC<ProductSearchProps> = ({ onResults, onFiltersChange }) => {
  ...
  return (
    <div className="bg-gradient-to-br from-gray-900 to-gray-800 rounded-2xl p-6 border border-holiday-gold/20 shadow-2xl">
      <div className="space-y-6">
        ...
        <div className="relative">
          <div className="relative">
            <input
              ...
              placeholder="Search for the best products across all marketplaces..."
              className="w-full bg-gray-800/50 border border-gray-700 rounded-xl px-6 py-4 pl-14 pr-32 text-white text-lg focus:outline-none focus:border-holiday-gold focus:ring-2 focus:ring-holiday-gold/20 transition-all"
            />
            <Search className="absolute left-5 top-5 w-6 h-6 text-holiday-gold" />
            <button
              onClick={handleSearch}
              disabled={isSearching || !filters.query.trim()}
              className="absolute right-3 top-2 bg-holiday-gold hover:bg-holiday-gold/90 disabled:bg-gray-600 disabled:cursor-not-allowed text-gray-900 px-6 py-2 rounded-lg font-semibold transition-colors flex items-center gap-2"
            >
              {isSearching ? (
              ...
              {suggestions.map((suggestion, index) => (
                ...
                >
                  <div className="flex items-center gap-3">
                    <TrendingUp className="w-4 h-4 text-holiday-gold" />
                    {suggestion}
                  </div>
                </button>
              ))}
        ...
        <div className="flex flex-wrap gap-3">
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-lg border transition-colors ${
              showAdvanced 
                ? 'bg-holiday-gold text-gray-900 border-holiday-gold' 
                : 'bg-gray-800 text-holiday-silver border-gray-700 hover:border-holiday-gold'
            }`}
          >
          ...
          <select
            ...
            className="px-4 py-2 bg-gray-800 border border-gray-700 rounded-lg text-white focus:outline-none focus:border-holiday-gold"
          >
        ...
        {showAdvanced && (
          <div className="bg-gray-800/30 rounded-xl p-4 border border-gray-700/50 space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              <div>
                <label className="block text-holiday-silver text-sm mb-2">Category</label>
                <select
                  value={filters.category}
                  onChange={(e) => setFilters(prev => ({ ...prev, category: e.target.value }))}
                  className="w-full px-3 py-2 bg-gray-800 border border-gray-700 rounded-lg text-white focus:outline-none focus:border-holiday-gold"
                >
              ...
              <div>
                <label className="block text-holiday-silver text-sm mb-2">
        ...
        {/* Popular Searches */}
        <div>
          <p className="text-holiday-silver text-sm mb-3">Popular Searches:</p>
          <div className="flex flex-wrap gap-2">
            {popularSearches.slice(0, 6).map((search, index) => (
              <button
                key={index}
                onClick={() => selectSuggestion(search)}
                className="px-3 py-1.5 bg-gray-800/50 hover:bg-gray-700/50 text-holiday-silver hover:text-white rounded-full text-sm border border-gray-700/50 hover:border-holiday-gold/50 transition-all"
              >
                {search}
              </button>
            ))}
